...
with st.form("second_form"):
    ...
    if runned:
        if(inputAlgo == "IP"):
            ...
            for i in range(ns):
                indexMe[i] = 'Shipment ' + str(i)
                if(len(fixArray(array, ns)[i]) != 0):
                    isSent[i] = True
                else:
                    isSent[i] = False
                arrayDF[i] = [isSent[i], fixArray(array, ns)[i]]
            dfObj = pd.DataFrame(arrayDF, columns=[
                'isDelievered', 'Path'], index=indexMe)
            st.dataframe(dfObj)
            ...
            st.dataframe(dfObj)
        if(inputType == "Upload a file"):
            testNumber = filename[len(filename) - 4]
            st.write('testNumber is: ', testNumber)
            if(inputAlgo == "Genetic"):
                draw_figure(testNumber, 1)
            elif(inputAlgo == "Greedy"):
                draw_figure(testNumber, 2)
            elif(inputAlgo == "DP"):
                draw_figure(testNumber, 3)
            elif(inputAlgo == "IP"):
                draw_figure(testNumber, 4)
